...
class Dat(DataCarrier):
    ...
    def __str__(self):
        return "OP2 Dat: %s on (%s) with dim %s and datatype %s" \
               % (self._name, self._set, self._dim, self._datatype)

    def __repr__(self):
        return "Dat(%r, %s, '%s', None, '%s')" \
               % (self._set, self._dim, self._datatype, self._name)
...
class Mat(DataCarrier):
    ...
    def __str__(self):
        return "OP2 Mat: %s, row set (%s), col set (%s), dimension %s, datatype %s" \
               % (self._name, self._sets[0], self._sets[1], self._dim, self._datatype)

    def __repr__(self):
        return "Mat(%r, %s, '%s', '%s')" \
               % (self._sets, self._dim, self._datatype, self._name)
...
class Map(object):
    ...
    def __str__(self):
        return "OP2 Map: %s from (%s) to (%s), dim %s " \
               % (self._name, self._from, self._to, self._dim)
